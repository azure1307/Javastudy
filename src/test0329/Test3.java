...
class Buyer {
	...
	void buy(Food f) { //리턴타입이 void인 메서드의 종료방법 (1) 마지막 중괄호 만남 (2) return 만남
		if (f.price > money) {
			System.out.println(f+"구매시 잔액 부족");
			return; //메서드 실행 중단
		} else {
			money -= f.price; //this.money
			point += f.point; //this.point
	...
	void summary() {
		int totPrice = 0;
		//구매목록은 list와 f,d,s list 총 4개의 String을 만들어서 cart[i]랑 "," 더해주기
		...
		System.out.println();
		System.out.println("총 구매금액 : "+totPrice);
		...
		System.out.print("과일 구매금액 : "+fruitPrice+", 과일 구매목록 : "+cart[0]
				+","+cart[1]);
		System.out.println();
		System.out.print("음료 구매금액 : "+drinkPrice+", 음료 구매목록 : "+cart[2]
				+","+cart[3]);
		System.out.println();
		System.out.print("과자 구매금액 : "+snackPrice+", 과자 구매목록 : "+cart[4]);
	}
}
//	총 구매금액 : 4500
